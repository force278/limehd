...
function ModalAdmin({
  ...
  useEffect(() => {
    if (modalAdminState) {
      axios
        .get(
          `http://localhost:8000/api/admin_users/${listAdminState[activeAdminState].id}`,
          {
            headers: {
    ...
    <div className={styles.modalStyle}>
      <div className={styles.formStyle}>
        ...
        <div className={styles.btnBox}>
          <button className={styles.deleteBtnStyle} onClick={delete_table_item}>
            {"Удалить"}
          </button>
          {enableSaveBtnState ? (
            <button
              className={styles.saveEnableBtnStyle}
              onClick={save_table_item}
            >
              {"Сохранить"}
            </button>
          ) : (
            <button
              className={styles.saveDisableBtnStyle}
              onClick={save_table_item}
            >
              {"Сохранено"}
            </button>
